...
img = cv2.imread('YF-A01.jpg')
#opencv BGR轉RGB
#img4 = cv2.cvtColor(img,cv2.COLOR_BGR2RGB)

thresh = 40
 
minBGR = np.array([img[0] - thresh, img[1] - thresh, img[2] - thresh])
maxBGR = np.array([img[0] + thresh, img[1] + thresh, img[2] + thresh])
...
hsv = cv2.cvtColor( np.uint8([[img]] ), cv2.COLOR_BGR2HSV)[0][0]
...
ycb = cv2.cvtColor( np.uint8([[img]] ), cv2.COLOR_BGR2YCrCb)[0][0]
...
lab = cv2.cvtColor( np.uint8([[img]] ), cv2.COLOR_BGR2LAB)[0][0]
